...
def parse_time(time_str: str) -> int:
    ...
    if len(parts) > 3:
        raise ValueError(
            f"Invalid time format: {time_str}. Use HH:MM:SS, MM:SS, or seconds"
        )
...
def validate_time_range(
    start_time: int, end_time: Optional[int], duration: Optional[int]
) -> Tuple[int, int]:
    """
...
def run_ffmpeg_with_timeout(ffmpeg_command: list[str], timeout_minutes: int = 30) -> int:
    ...
    print(f"FFmpeg process started (PID: {process.pid}), timeout: {timeout_minutes} minutes")
    ...
    try:
        while True:
            ...
            if elapsed_time > timeout_seconds:
                print(f"FFmpeg process timed out after {timeout_minutes} minutes")
                process.terminate()
            ...
            if poll_result is not None:
                print(f"FFmpeg completed with return code: {poll_result}")
                return poll_result
...
def cleanup_temp_file(downloaded_filename: str, verbose: bool = False) -> None:
    ...
    if verbose:
        print(f"Attempting to clean up temporary file: {downloaded_filename}")
        print(f"File exists: {temp_file.exists()}")
        if temp_file.exists():
            print(f"File size: {temp_file.stat().st_size} bytes")
    
    if not temp_file.exists():
        print(f"Temporary file already removed: {downloaded_filename}")
        return
    ...
    try:
        ...
        temp_file.unlink()
        print(f"Deleted temporary file: '{downloaded_filename}'")
        
        # Verify deletion
        if temp_file.exists():
            print(f"Warning: File still exists after deletion attempt: {downloaded_filename}")
        elif verbose:
            print("File successfully removed and verified")
        
    except PermissionError:
        print(f"Warning: Permission denied when deleting temporary file: {downloaded_filename}")
        print("You may need to delete it manually.")
    except FileNotFoundError:
        print(f"Temporary file already removed: {downloaded_filename}")
    except Exception as e:
        print(f"Warning: Could not delete temporary file '{downloaded_filename}': {e}")
...
def download_and_convert_video(
    ...
    if start_time is not None or end_time is not None or duration is not None:
        try:
            ...
            print(f"Error parsing time parameters: {e}")
            sys.exit(1)
    ...
    try:
        ...
        if Path(downloaded_filename).exists():
            print(f"Using existing temporary file: {downloaded_filename}")
        else:
        ...
        print(f"Converting '{downloaded_filename}' to '{final_output_path}'...")
        ...
        if return_code != 0:
            ...
            sys.exit(1)
        print(f"Conversion completed: '{final_output_path}'")
        ...
        print(
            "Error: FFmpeg not found. Please ensure FFmpeg is "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Download YouTube videos and convert them to MP4 format, "
            "with optional segment extraction."
        ),
...
Examples:
  ...
  python ytd.py "https://youtube.com/watch?v=..." -o "clip" --start 1:00 --end 3:45
    ...
    if (args.start or args.end or args.duration) and not args.start:
        print("Error: --start time is required when using segment extraction")
        sys.exit(1)
